...
class Meal:
    ...
    def __post_init__(self):
        """
        Validate the meal data after initialization.

        Raises:
            ValueError: If the price is negative or not one of the valid difficulties
        """
        if self.price < 0:
...
def create_meal(meal: str, cuisine: str, price: float, difficulty: str) -> None:
    ...
    Raises:
        ValueError: If the price is not valid number or the difficulty is not one of the valid options
...
def get_leaderboard(sort_by: str = "wins") -> dict[str, Any]:
    """
    Get leaderboard of the meal from sort by wins or win percentage.

    Args:
        sort_by (str): which way to sort the leaderboard.
    ...
    Returns:
        dict[str, Any]: The leaderboard sorted by the specified metric.
    """
    query = """
...
def get_meal_by_id(meal_id: int) -> Meal:
    ...
    Raises:
        ValueError: If the meal is not found.
        sqlite3.Error: If any database error occurs.

    Returns:
        Meal: The meal object retrieved from the catalog.
    """
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
...
def update_meal_stats(meal_id: int, result: str) -> None:
    """
    Update the battle stats for a meal based on the result.
    ...
    Raises:
        ValueError: If the meal is not found or if the result is invalid.
        sqlite3.Error: If any database error occurs.
    """
    try:
        with get_db_connection() as conn:
